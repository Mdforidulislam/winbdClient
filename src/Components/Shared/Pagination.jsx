    ...
    return (
        <div className='flex select-none justify-center items-center gap-5 mb-5'>
            ...
            <div className='flex justify-center items-center gap-2 '>
                {[...Array(page).keys()].map((item, ind) => <div onClick={() => { setPageNumber(item) }} className={`cursor-pointer hover:scale-110 scale-100 transition-all duration-200 px-5 ${pageNumber === item ? 'bg-[#3f4f8e] text-white' : 'bg-[#1e284e]'} border-sky-300  font-semibold text-white   py-3 rounded-full`} key={item}>
                    {item + 1}
                </div>)}
            </div>
            {/* right arrow */}
            <div onClick={() => { updatePageNumber(pageNumber + 1) }} className='bg-gray-200 hover:scale-110 scale-100 transition-all duration-200 cursor-pointer hover:bg-sky-200 px-4 py-4 rounded-full'>
