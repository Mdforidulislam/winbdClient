...
const Register = () => {
    ...
    /// get the author id from the url and store value local store and then access value register under author 
        const queryString = window.location.search;
        const urlParams = new URLSearchParams(queryString);
        const refParam = urlParams.get('ref');

        if (refParam) {
                    localStorage.setItem('registerId', refParam);
                }
        const authorId = localStorage.getItem('registerId');

    const onSubmit = async data => {
        console.log(data)
        // user register under the authentication 
        if (data.name && data.password) {
            const userData = { authorId: authorId, password: data.password, userName: data.name };
            console.log(userData,'check valu set to local store');
            const strigfyData = JSON.stringify(userData);
            localStorage.setItem('userData', strigfyData);
        }
        ...
        if (Object.values(userInfoList).every(item => item) || !Object.values(userInfoList).length === 0) {
            const registerUser = await axios.post('https://pay-winbd-server.vercel.app/insertUsers', userInfoList)
        ...
        <div className=" w-full h-full min-h-screen bg-[#0F0F11] ">
            ...
            <div className="">
                ...
                <div className="w-full h-full flex justify-center items-center py-4 bg-[#29292B] mb-3">
                    ...
                    <div className="mb-2 px-3 rounded-sm">
                                {/* fild number 1  */}
                        <div className='flex w-full bg-[#29292B] px-3 py-2 gap-3 items-center'>
                                <label htmlFor="" className="text-white text-md">username</label>
                         ...
                         <div className='flex w-full bg-[#29292B] px-3 py-2 gap-3 items-center border-t border-gray-500 '>
                            <label htmlFor="" className="text-white text-md capitalize">password</label>
                            <div className="">
                                <div>
                        ...
                        <div>
                            <div className=' flex w-full bg-[#29292B] px-3 py-2 gap-3 items-center border-t border-gray-500 '>
                                <label htmlFor="" className="text-white text-md capitalize">confirm password</label>
                                    <div>
                        ...
                        <div className=" px-3 w-full h-full mt-6">
                                    ...
                                    <div className="  flex w-full bg-[#29292B] px-3 py-2 gap-3 items-center">
                                        <label htmlFor="" className="text-white text-md capitalize">phone number</label>
                                            <div>
                               ...
                               <div className="flex w-full mt-5">
                                    ...
                                    <div className=" w-2/4">
                                        <h1 className="  w-full text-white flex justify-center items-center h-full bg-slate-600 px-1 text-md">Send OTP</h1>
                                    </div>
